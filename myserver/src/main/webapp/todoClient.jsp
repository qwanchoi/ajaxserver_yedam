...
function todoUpdate() {
	$('#myUL').on('click', 'li', function(e) {
		...
		let todoyn = $(this).attr('class')?'0':'1';
...
function todoInsert() {
	$('.addBtn').on('click', function(){
		...
		$.ajax({
			...
			todoList();
//			$('<li>')
//				.html(`\${r.contents}<span class="close">X</span>`)
//				.appendTo($('#myUL'));
			$('#myInput').val('');
		}); // end ajax
	}) // 
...
function todoList(){
	...
	$.ajax({
		...
		for(let v of list) {
			...
			let li = $('<li>')
				.attr('data-no', v.no)
				.html(v.contents+`<span class="close">X</span>`).appendTo($('#myUL'));
			if(v.todoyn == '1') {
